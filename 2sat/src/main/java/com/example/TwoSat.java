...
public final class TwoSat {
    ...
    public static <T> boolean isSatisfiable(List<Clause<T>> formula) {
        ...
        Map<Literal<T>, Boolean> truthAsgn = new HashMap<>();
        ...
        if(isSat) { // truth assignment based on the fact that scc is in topological order.

            int i = 0;
            while (!sccTruth.containsKey(i)) {
                for (Literal lit : rScc.get(i)) {
                    truthAsgn.put(lit, true);
                    //truthAsgn.put(lit.negation(), false);
                    sccTruth.put(scc.get(lit.negation()), false);
                }
                i++;
            }
// print out truth assignment

            for(T variable: variables){    // all true literals
                Literal<T> lit = new Literal<>(variable,true);
                if (truthAsgn.containsKey(lit))
                System.out.println(lit+":"+truthAsgn.get(lit));
                else System.out.println(lit+":"+false);
            }
